...
class TCharBufferWasmGC extends TCharBufferNative {
    TCharBufferWasmGC(char[] array, int position, int limit, boolean readOnly, Object base, Address address,
    ...
    TCharBuffer duplicate(int start, int capacity, int position, int limit, boolean readOnly) {
        return new TCharBufferWasmGC(array, position, limit, readOnly, base, address.add(start * 2), capacity, swap);
    }
