...
final class THexStringParser {
    ...
    private static final TPattern PATTERN = TPattern.compile(HEX_PATTERN);
    ...
    private long parse(String hexString, boolean isDouble) {
        TMatcher matcher = PATTERN.matcher(hexString);
        if (!matcher.matches()) {
    ...
    private void round() {
        ...
        boolean moreThanZero = !result.isEmpty();
