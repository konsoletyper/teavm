...
public class TWeakHashMap<K, V> extends TAbstractMap<K, V> implements TCloneable, TSerializable {
    ...
    private static class AbstractMapIterator<K, V>  {
        ...
        final TWeakHashMap<K, V> associatedMap;

        AbstractMapIterator(TWeakHashMap<K, V> hm) {
            associatedMap = hm;
    ...
    private static class EntryIterator<K, V> extends AbstractMapIterator<K, V>
        ...
        EntryIterator(TWeakHashMap<K, V> map) {
            super(map);
        }
    ...
    private static class KeyIterator<K, V> extends AbstractMapIterator<K, V> implements TIterator<K> {
        KeyIterator(TWeakHashMap<K, V> map) {
            super(map);
        }
    ...
    private static class ValueIterator<K, V> extends AbstractMapIterator<K, V> implements TIterator<V> {
        ValueIterator(TWeakHashMap<K, V> map) {
            super(map);
        }
    ...
    static class HashMapEntrySet<K, V> extends TAbstractSet<TMap.Entry<K, V>> {
        private final TWeakHashMap<K, V> associatedMap;

        public HashMapEntrySet(TWeakHashMap<K, V> hm) {
            associatedMap = hm;
        }

        TWeakHashMap<K, V> hashMap() {
    ...
    public TObject clone0() {
        try {
            TWeakHashMap<K, V> map = (TWeakHashMap<K, V>) super.clone();
            map.elementCount = 0;
        ...
        if (cachedKeySet == null) {
            cachedKeySet = new TAbstractSet<K>() {
                ...
                @Override public boolean remove(Object key) {
                    HashEntry<K, V> entry = TWeakHashMap.this.removeEntry(key);
                    return entry != null;
                }
                @Override public TIterator<K> iterator() {
                    return new KeyIterator<>(TWeakHashMap.this);
                }
        ...
        if (cachedValues == null) {
            cachedValues = new TAbstractCollection<V>() {
                ...
                @Override public TIterator<V> iterator() {
                    return new ValueIterator<>(TWeakHashMap.this);
                }
