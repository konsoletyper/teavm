...
public class ReferenceResolver {
    ...
    private boolean resolveSpecial(InvokeInstruction instruction, MethodReference calledRef) {
        ...
        if (!checkMethod(instruction.getLocation(), resolvedMethod.getReference())) {
            return false;
        }
    ...
    private boolean checkClass(Instruction instruction, String className) {
        ...
        if (!checkPlatformSupported(cls.getAnnotations())) {
            ...
            emitExceptionThrow(instruction.getLocation(), NoClassDefFoundError.class.getName(), "Class not found: " + className);
            truncateBlock(instruction);
            return false;
        }
        return true;
    ...
    private boolean checkMethod(TextLocation location, MethodReference methodRef) {
        ...
        if (cls != null) {
            ...
            if (methodReader != null && !checkPlatformSupported(methodReader.getAnnotations())) {
                reportError(location, "Method {{m0}} is not supported on current target", methodRef);
                emitExceptionThrow(location, NoSuchMethodError.class.getName(), "Method not found: " + methodRef);
                return false;
            }
        }
        return true;
