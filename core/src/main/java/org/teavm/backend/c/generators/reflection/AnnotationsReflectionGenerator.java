...
public class AnnotationsReflectionGenerator implements ReflectionGenerator {
    ...
    private ReflectionPartGenerator generateAnnotationValue(String targetName, ReflectionGeneratorContext context,
        ...
        switch (value.getType()) {
            ...
            case AnnotationValue.ENUM: {
                var fieldName = context.globalContext().getNames().forStaticField(value.getEnumValue());
                context.includes().includeClass(value.getEnumValue().getClassName());
                var enumCls = context.globalContext().getClassSource().get(value.getEnumValue().getClassName());
                if (enumCls != null && enumCls.getMethod(new MethodDescriptor("<clinit>", ValueType.VOID)) != null) {
                    if (!context.globalContext().getCharacteristics().isStaticInit(enumCls.getName())) {
                        var initName = context.globalContext().getNames().forClassInitializer(
                                value.getEnumValue().getClassName());
                        context.addToInitializer(w -> w.print(initName).println("();"));
                    }
                }

                return w -> w.print(fieldName);
            }
    ...
    private String generateArray(String targetName, ReflectionGeneratorContext context, ValueType.Array type,
        ...
        context.addTopLevel(w -> {
            ...
            w.printType(type.getItemType()).print(" data[").print(Integer.toString(elementGenerators.size()))
                    .println("]");
            w.outdent().print("} ").print(arrayVarName).print(" = {").indent();
            w.println(".data = { ").indent();
        ...
        context.addToInitializer(w -> {
            var instanceName = names.forClassInstance(type);
            w.print(arrayVarName).print(".parent.header = TEAVM_PACK_CLASS(&").print(instanceName).println(");");
        });
