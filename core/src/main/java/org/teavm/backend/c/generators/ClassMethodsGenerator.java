...
public class ClassMethodsGenerator implements Generator {
    ...
    private void generateGetDeclaredAnnot(GeneratorContext context) {
        ...
        context.includes().includeType(objectType);
        ...
        writer.print("return ")
                .print(context.names().forMethod(ALLOC_ARRAY_METHOD)).print("(&")
                .print(context.names().forClassInstance(objectType))
                .print(", 0);").println();
        writer.outdent().println("}");

        writer.print("TeaVM_Array* array = ").print(context.names().forMethod(ALLOC_ARRAY_METHOD))
                .print("(&").print(context.names().forClassInstance(objectType))
                .print(", reflect->annotationCount);").println();
