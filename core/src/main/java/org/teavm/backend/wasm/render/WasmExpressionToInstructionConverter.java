...
class WasmExpressionToInstructionConverter implements WasmExpressionVisitor {
    private WasmInstructionList target;
    private final Map<WasmBlock, WasmInstructionList> blockMapping = new HashMap<>();

    WasmExpressionToInstructionConverter(WasmInstructionList target) {
        this.target = target;
    }

    void convert(WasmExpression expression) {
        expression.acceptVisitor(this);
    }

    private void convertAll(Iterable<WasmExpression> expressions) {
    ...
    public void visit(WasmBlock expression) {
        ...
        blockMapping.put(expression, block);
        convertInto(block, expression.getBody());
    ...
    public void visit(WasmInt32Constant expression) {
        var insn = new WasmInt32ConstantInstruction();
        insn.setValue(expression.getValue());
        insn.setLocation(expression.getLocation());
        target.add(insn);
    }
    ...
    public void visit(WasmInt64Constant expression) {
        var insn = new WasmInt64ConstantInstruction();
        insn.setValue(expression.getValue());
        insn.setLocation(expression.getLocation());
        target.add(insn);
    }
    ...
    public void visit(WasmFloat32Constant expression) {
        var insn = new WasmFloat32ConstantInstruction();
        insn.setValue(expression.getValue());
        insn.setLocation(expression.getLocation());
        target.add(insn);
    }
    ...
    public void visit(WasmFloat64Constant expression) {
        var insn = new WasmFloat64ConstantInstruction();
        insn.setValue(expression.getValue());
        insn.setLocation(expression.getLocation());
        target.add(insn);
    }
    ...
    public void visit(WasmTry expression) {
        ...
        convertInto(tryInsn, expression.getBody());
        for (var catchClause : expression.getCatches()) {
            var clause = new WasmCatchClause(catchClause.getTag());
            clause.getCatchVariables().addAll(catchClause.getCatchVariables());
