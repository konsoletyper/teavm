...
class WasmBinaryInstructionRenderingVisitor implements WasmInstructionVisitor {
    ...
    private int depth;
    private final Map<WasmInstructionList, Integer> blockDepths = new HashMap<>();

    WasmBinaryInstructionRenderingVisitor(WasmBinaryWriter writer, WasmModule module) {
        this.writer = writer;
        this.module = module;
    }
    ...
    public void visit(WasmConditionalInstruction instruction) {
        ...
        writeBlockType(instruction.getType());
        ++depth;
        blockDepths.put(instruction.getThenBlock(), depth);
        render(instruction.getThenBlock());
        blockDepths.remove(instruction.getThenBlock());
        if (!instruction.getElseBlock().isEmpty()) {
            writer.writeByte(0x05);
            blockDepths.put(instruction.getElseBlock(), depth);
            render(instruction.getElseBlock());
            blockDepths.remove(instruction.getElseBlock());
        }
        --depth;
        writer.writeByte(0x0B);
    }
    ...
    public void visit(WasmBranchInstruction instruction) {
        ...
        writeLabel(instruction.getTarget());
    }
    ...
    public void visit(WasmSwitchInstruction instruction) {
        ...
        for (var target : instruction.getTargets()) {
            writer.writeLEB(depth - blockDepths.get(target));
        }
        writer.writeLEB(depth - blockDepths.get(instruction.getDefaultTarget()));
    }
    ...
    private void writeLabel(WasmInstructionList target) {
        writer.writeLEB(depth - blockDepths.get(target));
    }
